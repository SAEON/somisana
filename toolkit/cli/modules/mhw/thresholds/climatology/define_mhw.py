...
def min_time_array(x):
    if all(np.isnan(x)):
        ...
        mh_reshape_len = np.repeat(
            999,
            np.nanmax(mh_array[:, 1] * len(mh_array)),
        ).reshape(len(mh_array), int(np.nanmax(mh_array[:, 1])))
        ...
        n_rows = np.shape(mh_reshape_len)[0]
        n_cols = np.shape(mh_reshape_len)[1]
        ...
        n_rows = np.shape(mh_reshape)[0]
        n_cols = np.shape(mh_reshape)[1]
...
def calculate_mhw(mhw_bulk_cache, nc_thresholds_path, nc_mhw_output_path):
    ...
    row_length = np.shape(threshold_sst[0, :, :])[0]
    rows = np.arange(start=1, stop=row_length, step=1)
    column_length = np.shape(threshold_sst[0, :, :])[1]
    columns = np.arange(start=1, stop=column_length, step=1)
    ...
    ds = xr.Dataset(
        {
            ...
            "marine_heat_wave_length": (
                ["time", "latitude", "longitude"],
                marine_heat_wave_length,
            ),
            "marine_heat_wave_category": (
                ["time", "latitude", "longitude"],
                marine_heat_wave_category,
            ),
    ...
    ds.climatology_values.attrs[
        "comment"
    ] = "The daily climatology calculated on a 31 day moving average"
    ds.climatology_values.attrs["units"] = "^oC"

    ds.threshold_values.attrs[
        "comment"
    ] = "The daily climatology calculated on a 31 day moving average"
    ds.threshold_values.attrs["units"] = "^oC"

    encoding = {
        "marine_heat_waves": {"dtype": "int16"},
        "marine_heat_wave_length": {"dtype": "int16"},
        "marine_heat_wave_category": {"dtype": "int16"},
    }

    # Saving dataset to saving path and filename
    ds.to_netcdf(nc_mhw_output_path, encoding=encoding)
