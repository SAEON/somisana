...
"""
This is the official 'package' or script used to calculate mhw from (https://github.com/ecjoliver/marineHeatWaves)
The function detect requires a 1d array of sst and time and calculates mhw over the full period.
The function requires the full period of sst available.
"""
...
def detect(
    ...
    Options:
      ...
      climatologyPeriod      Period over which climatology is calculated, specified
                             ...
                             Alternate periods supplied as a list e.g. [1983,2012].
    ...
    Written by Eric Oliver, Institute for Marine and Antarctic Studies, University of Tasmania, Feb 2015
...
def blockAverage(t, mhw, clim=None, blockLength=1, removeMissing=False, temp=None):
    ...
    if sw_cats:
        ...
        for ev in range(mhw["n_events"]):
            mhwIndex[mhw["index_start"][ev] : mhw["index_end"][ev] + 1] = 1.0
    ...
    for i in range(mhw["n_events"]):
        ...
        iBlock = np.where(
            (mhwBlock["years_start"] <= mhw["date_start"][i].year)
            * (mhwBlock["years_end"] >= mhw["date_start"][i].year)
        )[0][0]
        ...
        mhwBlock["intensity_max_max"][iBlock] = np.max(
            [mhwBlock["intensity_max_max"][iBlock], mhw["intensity_max"][i]]
        )
        ...
        mhwBlock["intensity_mean_relThresh"][iBlock] += mhw["intensity_mean_relThresh"][
            i
        ]
        mhwBlock["intensity_cumulative_relThresh"][iBlock] += mhw[
        ...
        mhwBlock["intensity_cumulative_abs"][iBlock] += mhw["intensity_cumulative_abs"][
            i
        ]
        ...
        if mhw["date_start"][i].year == mhw["date_end"][i].year:  # MHW in single year
            ...
            for yr_mhw in np.unique(year_mhw):
                iBlock = np.where(
                    (mhwBlock["years_start"] <= yr_mhw)
                    * (mhwBlock["years_end"] >= yr_mhw)
                )[0][0]
    ...
    if sw_cats:
        for i in range(int(nBlocks)):
            mhwBlock["moderate_days"][i] = (
                (
                    (year >= mhwBlock["years_start"][i])
                    * (year <= mhwBlock["years_end"][i])
                    * mhwIndex
                    * (cats == 1)
                )
                .astype(int)
    ...
    mhwBlock["intensity_cumulative_relThresh"] = (
        mhwBlock["intensity_cumulative_relThresh"] / count
    )
    ...
    if sw_temp:
        for i in range(int(nBlocks)):
            tt = (year >= mhwBlock["years_start"][i]) * (
                year <= mhwBlock["years_end"][i]
            )
    ...
    if removeMissing:
        ...
        for y in range(len(missingYears)):
            iMissing = np.where(
                (mhwBlock["years_start"] <= missingYears[y])
                * (mhwBlock["years_end"] >= missingYears[y])
            )[0][0]
...
def meanTrend(mhwBlock, alpha=0.05):
    ...
    for key in mhwBlock.keys():
        ...
        Sxx = np.sum(X[valid, 1] ** 2) - (np.sum(X[valid, 1]) ** 2) / len(
            t[valid]
        )  # np.var(X, axis=1)[1]
