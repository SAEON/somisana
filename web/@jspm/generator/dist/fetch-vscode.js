import { fetch as fetch$1 } from './fetch-native.js';
export { clearCache } from './fetch-native.js';

function sourceResponse(buffer) {
    return {
        status: 200,
        async text () {
            return buffer.toString();
        },
        async json () {
            return JSON.parse(buffer.toString());
        },
        arrayBuffer () {
            return buffer.buffer || buffer;
        }
    };
}
const dirResponse = {
    status: 200,
    async text () {
        return '';
    },
    async json () {
        throw new Error('Not JSON');
    },
    arrayBuffer () {
        return new ArrayBuffer(0);
    }
};
// @ts-ignore
const vscode = require('vscode');
const fetch = async function(url, opts) {
    if (!opts) throw new Error('Always expect fetch options to be passed');
    const urlString = url.toString();
    const protocol = urlString.slice(0, urlString.indexOf(':') + 1);
    switch(protocol){
        case 'ipfs:':
            throw new Error('IPFS Support for VSCode not yet implemented');
        case 'file:':
            if (urlString.endsWith('/')) {
                try {
                    await vscode.workspace.fs.readFile(vscode.Uri.parse(urlString));
                    return {
                        status: 404,
                        statusText: 'Directory does not exist'
                    };
                } catch (e) {
                    if (e.code === 'FileIsADirectory') return dirResponse;
                    throw e;
                }
            }
            try {
                return sourceResponse(new TextDecoder().decode(await vscode.workspace.fs.readFile(vscode.Uri.parse(urlString))));
            } catch (e1) {
                if (e1.code === 'FileIsADirectory') return dirResponse;
                if (e1.code === 'Unavailable' || e1.code === 'EntryNotFound' || e1.code === 'FileNotFound') return {
                    status: 404,
                    statusText: e1.toString()
                };
                return {
                    status: 500,
                    statusText: e1.toString()
                };
            }
        case 'data:':
        case 'http:':
        case 'https:':
            // @ts-ignore
            return fetch$1(url, opts);
    }
};

export { fetch };
