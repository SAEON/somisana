...
class Resolver {
    ...
    async finalizeResolve(url, parentIsCjs, env, installer, pkgUrl) {
        ...
        if (installer && url.startsWith('node:')) {
            const subpath = `./nodelibs/${url.slice(5)}`;
    ...
    async getExportResolution(pkgUrl, subpath, originalSpecifier) {
        ...
        if (pcfg.exports !== undefined && pcfg.exports !== null) {
            if (typeof pcfg.exports === 'string') {
                ...
                for (const curTarget of targets){
                    try {
                        if (await this.finalizeResolve(new URL(curTarget, pkgUrl).href, false, [], null, pkgUrl) === pkgUrl + subpath.slice(2)) return '.';
                    } catch  {}
                }
                return null;
            } else {
    ...
    async resolveExport(pkgUrl, subpath, env, parentIsCjs, originalSpecifier, installer, parentUrl) {
        ...
        if (pcfg.exports !== undefined && pcfg.exports !== null) {
            ...
            } else if (!allDotKeys1(pcfg.exports)) {
...
class Generator {
   ...
   */ async traceInstall(specifier1, parentUrl) {
        if (typeof specifier1 === 'string') specifier1 = [
            specifier1
        ];
        let error = false;
        if ((this.installCnt++) === 0) this.traceMap.startInstall();
        specifier1 = specifier1.map((specifier)=>specifier.replace(/\\/g, '/')
        );
        await this.traceMap.processInputMap;
        try {
            await Promise.all(specifier1.map((specifier)=>this.traceMap.visit(specifier, {
                    mode: 'new',
                    toplevel: true
                }, parentUrl || this.mapUrl.href)
            ));
            for (const s of specifier1){
   ...
   */ async install(install1) {
        ...
        try {
            ...
            if (typeof install1 === 'string' || typeof install1.target === 'string') {
                ({ alias , target , subpath  } = await installToTarget.call(this, install1, this.traceMap.installer.defaultRegistry));
            } else {
                ({ alias , target , subpath  } = install1);
            }
    ...
    async update(pkgNames) {
        ...
        for (const name of pkgNames){
            ...
            const subpaths = this.traceMap.pins.filter((pin)=>pin === name || pin.startsWith(name) && pin[name.length] === '/'
            ).map((pin)=>`.${pin.slice(name.length)}`
            );
