...
function parseUrlPkg$6(url) {
    ...
    const [, , registry, name, version] = url.slice((layer === 'default' ? cdnUrl$4 : systemCdnUrl).length).match(exactPkgRegEx$3) || [];
...
async function checkBuildOrError(pkg, fetchOpts) {
    ...
    const pjsonRes = await fetch$1(`${cdnUrl$4}${pkgStr}/package.json`, fetchOpts);
    if (pjsonRes.ok) return true;
    // no package.json! Check if there's a build error:
    const errLogRes = await fetch$1(`${cdnUrl$4}${pkgStr}/_error.log`, fetchOpts);
    if (errLogRes.ok) {
    ...
    console.error(`Unable to request ${cdnUrl$4}${pkgStr}/package.json - ${pjsonRes.status} ${pjsonRes.statusText || 'returned'}`);
    return false;
}
...
function pkgToUrl$5(pkg) {
    if (pkg.registry !== 'node') return pkgToUrl$6(pkg, 'default');
...
async function resolveLatestTarget$2(target, unstable, _layer, parentUrl) {
    let resolved = await resolveLatestTarget$3.call(this, {
        registry: 'npm',
        name: '@jspm/core',
...
function parseUrlPkg$4(url) {
    ...
    } else if (url.startsWith(cdnUrl$3)) {
        const path = url.slice(cdnUrl$3.length);
...
async function resolveLatestTarget$1(target, unstable, _layer, parentUrl) {
    const { registry , name , range  } = target;
    ...
    const res = await fetch$1((registry === 'denoland' ? cdnUrl$3 : stdlibUrl + '/') + name + '/mod.ts', fetchOpts);
    if (!res.ok) throw new Error(`Deno: Unable to lookup ${(registry === 'denoland' ? cdnUrl$3 : stdlibUrl + '/') + name}`);
    return {
        pkg: parseUrlPkg$4(res.url).pkg,
...
async function resolveLatestTarget(target, _unstable, _layer, parentUrl) {
...
let babel$1, babelPresetTs, babelPluginImportAssertions;
...
async function createTsAnalysis(source, url) {
    if (!babel$1) [{ default: babel$1  }, { default: { default: babelPresetTs  }  }, { default: babelPluginImportAssertions  }] = await Promise.all([
        import('@babel/core'),
                ...
                ({ types: t  })=>{
                    return {
                        visitor: {
                            ...
                            Import (path) {
                                dynamicImports.add(buildDynamicString$1(path.parentPath.get('arguments.0').node, url, true));
                            },
                            MetaProperty (path) {
...
export { Generator, analyzeHtml, clearCache, fetch, getPackageBase, getPackageConfig, lookup };
